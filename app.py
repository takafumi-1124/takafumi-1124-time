...
with tabs[3]:
    ...
    st.caption("""
    ※ 各カテゴリのスコアは、あなたが測定したESGの重要度をもとに、企業の環境・社会・ガバナンスの取り組みが合計スコアにどれだけ影響しているかを示した「寄与スコア」です。
　　※ 合計スコアは、環境・社会・ガバナンスの寄与スコアを合計した値です。
　　※ 企業名をクリックすると、その企業のホームページを開くことができます。
 　 ※ 寄与スコアが大きいカテゴリほど、あなたの重視したESG項目がその企業評価に強く反映されています。
    """)
    st.markdown("""
    ※ 各カテゴリのスコアは、あなたが測定したESGの重要度をもとに、  
    企業の環境・社会・ガバナンスの取り組みが  
    合計スコアにどれだけ影響しているかを示した「寄与スコア」です。  
    
    ※ 合計スコアは、環境・社会・ガバナンスの寄与スコアを合計した値です。  
    
    ※ 企業名をクリックすると、その企業のホームページを開くことができます。  
    
    ※ 寄与スコアが大きいカテゴリほど、あなたの重視したESG項目が  
    その企業評価に強く反映されています。
    """)

    st.caption(
    "※ 各カテゴリのスコアは、あなたが測定したESGの重要度をもとに、企業の環境・社会・ガバナンスの取り組みが合計スコアにどれだけ影響しているかを示した「寄与スコア」です。<br>"
    "※ 合計スコアは、環境・社会・ガバナンスの寄与スコアを合計した値です。<br>"
    "※ 企業名をクリックすると、その企業のホームページを開くことができます。<br>"
    "※ 寄与スコアが大きいカテゴリほど、あなたの重視したESG項目がその企業評価に強く反映されています。"
)
